...
public class RequestHandler implements Runnable {
    ...
    public RequestHandler(RequestChannel channel) {
        //TODO#8-8 RequestChannel == null 이면 IllegalArgumentException이 발생 합니다.
        if(Objects.isNull(channel)){
            throw new IllegalArgumentException();
        }
        //TODO#8-9 channel을 초기화 합니다.
        this.channel = channel;
    }
    
    @Override
    public void run() {
        /*TODO#8-10 while 조건문을 수정 하세요
            - interrupt가 발생하면 while문이 종료 되면서 thread를 빠져 나가게 됩니다.
         */
        while(!Thread.currentThread().isInterrupted()){
            try{
                //TODO#8-11 channel.getRequest()를 호출해서 execute() method를 실행 합니다.
                Executable executable = channel.getRequest();
                executable.execute();
                Thread.sleep(100);
            }catch (Exception e){
                //TODO#8-12 interruptedException이 발생하면 interrupted flag 값이 false 상태로 설정 됩니다. 'Thread.currentThread().interrupt()'를 호출하여 다시 true상태로 변경 합니다.
