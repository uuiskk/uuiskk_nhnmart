...
public class ThreadPool {
    ...
    public ThreadPool(Runnable runnable){
        //TODO#8-1 default 생성자 구현, poolSize = DEFAULT_POOL_SIZE를 사용합니다.
        this(DEFAULT_POOL_SIZE,runnable);
    }

    public ThreadPool(int poolSize, Runnable runnable) {
        //TODO#8-2 thread pool size <0 다면 IllegalArgumentException이 발생 합니다.
        if(poolSize<0){
            throw new IllegalArgumentException();
        }

        //TODO#8-3 runable == null 이면 IllegalArgumentException 발생 합니다.
        if(Objects.isNull(runnable)){
            throw new IllegalArgumentException();
        }

        //TODO#8-4 runnable 이 Runnable의 구현체가 아니라면 IllegalArgumentException 발생 합니다.
        if(!(runnable instanceof Runnable)){
            throw new IllegalArgumentException();
        }

        //TODO#8-5 poolSize, runnable, threadList 초기화
    ...
    public synchronized void start(){
        /*TODO#8-6 thread를생성 -> threadList에 등록 합니다. -> 생성된 thread를 시작 합니다.
    ...
    public synchronized void stop(){
        /*TODO#8-7 interrupt()를 실행해서 thread를 종료 합니다.
        ...
        for(Thread thread : threadList){
            try {
                //join method는 해당 thread가 종료될 때 까지 현재 thread를 대기상태로 만듬니다.
                thread.join();
